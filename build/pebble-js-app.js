Pebble.addEventListener("ready", function(e) {
  ...
  var settings;
  try {
    settings = JSON.parse(localStorage['Handwritten']);
    Pebble.sendAppMessageWithSizeCheck(settings);
  }
  catch(e) {
    // the dicitonary was never stored, the watchapp uses its default values
    settings = {};
    localStorage['Handwritten'] = JSON.stringify(settings);
  }
  
});

Pebble.addEventListener("showConfiguration", function (e) {
  var url = "http://etiennovski.github.io/Handwritten?";

  var settings = JSON.parse(localStorage['Handwritten']);
...
Pebble.addEventListener("webviewclosed", function(e) {
    ...
    if ((typeof e.response === 'string') && (e.response.length > 0)) {
        ...
        var diffSettings = diff(newSettingsDictionary, JSON.parse(localStorage['Handwritten']));
        
        localStorage['Handwritten'] = newSettings;
